...
- Initial scroll: a mount-only `useLayoutEffect` sets `scrollLeft` so today sits `TODAY_LEAD_IN` (7) days in from the left, instead of opening on `rangeStart` (= earliest task date − `RANGE_PAD`). Mount-only is deliberate — an SSE hot-reload must not move the user's scroll position. Roadmaps entirely in the past or future need no special case: the browser clamps `scrollLeft`, landing on the right edge or staying at the left.
